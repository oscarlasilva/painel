...
function makeKpis(rows, byPeriod, periods) {
  ...
  document.getElementById("updateBadge").textContent = `Última competência: ${monthLabel(latest)}`;
  const latestRows=[latestAPS,latestESP].filter(Boolean);
  const partial=latestRows.some(r => r.fila_final == null || r.entrada == null);
  document.getElementById("notice").innerHTML = partial
...
function renderTables(rows) {
  ...
  document.getElementById("performanceTable").innerHTML=`<thead><tr><th>Período</th><th>Serviço</th><th class="num">Taxa de resolução</th><th class="num">Saldo</th><th class="num">Fila final</th></tr></thead><tbody>`+
  complete.map(r=>{const res=r.saida/r.fila_anterior*100,saldo=r.entrada-r.saida;return `<tr><td>${monthLabel(r.periodo)}</td><td class="service">${r.tipo}</td><td class="num">${pct(res)}</td><td class="num">${saldo>0?"+":""}${fmtInt(saldo)}</td><td class="num">${fmtInt(r.fila_final)}</td></tr>`}).join("")+"</tbody>";

  const sorted=[...rows].sort((a,b)=>periodKey(b.periodo)-periodKey(a.periodo)||a.tipo.localeCompare(b.tipo));
  document.getElementById("dataTable").innerHTML=`<thead><tr><th>Período</th><th>Serviço</th><th>Data-base</th><th class="num">Fila anterior</th><th class="num">Entradas</th><th class="num">Permanecem</th><th class="num">Saídas</th><th class="num">Fila final</th><th class="num">Média espera</th><th class="num">Máxima espera</th><th>Status</th></tr></thead><tbody>`+
...
async function init() {
  try {
    ...
    lineChart(document.getElementById("queueChart"),periods.map(monthLabel),[
      {name:"APS",values:periods.map(p=>byPeriod[p]?.APS?queueValue(byPeriod[p].APS):null)},
      {name:"Especializada",values:periods.map(p=>byPeriod[p]?.Especializada?queueValue(byPeriod[p].Especializada):null)}
    ]);
